...
class article extends database
{
    ...
    public function getArticleByIdUsers(){
        $req = 'SELECT COUNT(Commentaires.id) AS nbrcom, Articles.id, pseudo, msg, avatars, DATE_FORMAT(dateArticles, "%d/%m/%Y %H:%i") as dateArticles, utilisateurs.id AS id_user FROM Articles LEFT JOIN utilisateurs ON Articles.id_utilisateurs = utilisateurs.id LEFT JOIN Commentaires ON Articles.id = Commentaires.id_Articles WHERE Articles.id_utilisateurs = :id GROUP BY Articles.id ORDER BY Articles.id DESC ';
        $article = $this->db->prepare($req);
    ...
    {
        $query = 'SELECT COUNT(Commentaires.id) AS nbrcom, Articles.id, pseudo, msg, avatars, DATE_FORMAT(dateArticles, "%d/%m/%Y %H:%i") as dateArticles, utilisateurs.id as id_auteur FROM Articles LEFT JOIN utilisateurs ON Articles.id_utilisateurs = utilisateurs.id LEFT JOIN Commentaires ON Articles.id = Commentaires.id_Articles GROUP BY Articles.id ORDER BY Articles.id DESC LIMIT 4 OFFSET :offset  ;';
    ...
    public function searchArticleByUser($recherche){
        $rechercheArticle ='SELECT COUNT(Commentaires.id) AS nbrcom, Articles.`id`, `msg`, `dateArticles`, `pseudo`, avatars FROM `Articles` INNER JOIN utilisateurs ON Articles.id_utilisateurs = utilisateurs.id LEFT JOIN Commentaires ON Articles.id = Commentaires.id_Articles WHERE msg LIKE ? AND Articles.id_utilisateurs = ? GROUP BY Articles.id';
    ...
    public function searchAllArticle($recherche){
        $rechercheArticle = 'SELECT COUNT(Commentaires.id) AS nbrcom, Articles.`id`, `msg`, `dateArticles`, `pseudo`, nom , avatars , utilisateurs.id AS id_auteur FROM `Articles` INNER JOIN utilisateurs ON Articles.id_utilisateurs = utilisateurs.id INNER JOIN Roles ON Roles.id = utilisateurs.id_Roles LEFT JOIN Commentaires ON Articles.id = Commentaires.id_Articles  WHERE msg LIKE ? GROUP BY Articles.id  ';
        $article = $this->db->prepare($rechercheArticle);
