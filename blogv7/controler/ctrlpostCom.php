...
if(isset($_POST['btnCom'])){
 ...
 if(isset($_GET['Articleid'])&& !empty($_GET['Articleid'])&& is_numeric($_GET['Articleid'])){
    $com->id_Articles = $_GET['Articleid'];
...
if(isset($_POST['btnCloture']) && isset($_GET['Articleid'])){
   $article->id = $_GET['Articleid'];
   $etat = 1;
   $article->etatArticle = $etat;
   $article->changeEtat();
   header('location: index.php?postCom&Articleid='.$_GET['Articleid'].'&etat='.$etat);
}
