...
if (isset($_POST['envoi'])) {
    ...
    if (!empty($_POST['pseudo']) && !empty($_POST['mdp']) && !empty($_POST['email'])) {
       ...
       echo $formError['pseudo'] = "pseudo non remplit! <br>";
       echo $formError['mdp'] = "mdp non remplit! <br>";
       echo $formError['mail'] = "mail non remplit!";
    }
    ...
    if (empty($formError)) {
        ...
        if(is_object($verifuser)){
          if ($verifuser->pseudo === $pseudo && $verifuser->email === $mail ){
              echo "vous avez deja un compte chez nous";
          }else if($verifuser->pseudo === $pseudo){
            echo "le pseudo existe deja";
          }else if($verifuser->email === $mail){
...
/*if (isset($_POST['envoi'])) {
    ...
    if (!empty($_POST['pseudo']) && isset($_POST['pseudo'])) {
        ...
        $formError['pseudo'] = "pseudo non remplit!";
    }
    if (!empty($_POST['mdp']) && isset($_POST['mdp'])) {
        $mdp = sha1($_POST['mdp']);
    } else {
        $formError['mdp'] = "mdp non remplit!";
    }
    if (!empty($_POST['mail']) && isset($_POST['mail'])) {
        $mail = htmlspecialchars($_POST['mail']);
    } else {
        $formError['mail'] = "mail non remplit!";
    }
