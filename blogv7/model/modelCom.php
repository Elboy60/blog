...
class commentaire extends database{
    ...
    {
        $query = 'SELECT pseudo , avatars , commentaire, Commentaires.id, id_utilisateurs as id_auteur FROM commentaires LEFT JOIN utilisateurs ON Commentaires.id_utilisateurs = utilisateurs.id WHERE id_Articles = :id_Articles ORDER BY Commentaires.id DESC  LIMIT 4 OFFSET :offset ;';
    ...
    public function nbrComByArticle(){
        $req = 'SELECT COUNT(id) FROM Commentaires GROUP BY id_Articles';
        $numbrcom = $this->db->prepare($req);
        $numbrcom->execute();
        return $numbrcom->fetchAll(PDO::FETCH_OBJ);
    }

    public function countAllCom(){
        $req = 'SELECT COUNT(Commentaires.id) as countcom FROM Commentaires WHERE id_utilisateurs = :id_utilisateurs';
        $article = $this->db->prepare($req);
        $article->bindValue(':id_utilisateurs', $this->id_utilisateurs, PDO::PARAM_INT);
        $article->execute();
